...
async function loginWithFacebook() {
    if (!FACEBOOK_APP_ID) {
        showError('Facebook login chua duoc cau hinh.');
        return;
    }
    ...
    window.FB.login(async (response) => {
        if (!response.authResponse?.accessToken) {
            showError('Khong nhan duoc token Facebook.');
            return;
        }
        try {
...
export function renderLogin(mode = 'login') {
    ...
    appElement.innerHTML = `
        <div class="text-center mb-2">
            <h2>Sonic Parent</h2>
            <p>${isRegister ? 'Tao tai khoan phu huynh' : 'Dang nhap de quan ly tien do hoc tap cua be'}</p>
        </div>

        <div class="auth-tabs mb-2">
            <button type="button" id="showLogin" class="auth-tab ${!isRegister ? 'active' : ''}">Dang nhap</button>
            <button type="button" id="showRegister" class="auth-tab ${isRegister ? 'active' : ''}">Dang ky</button>
        </div>

        <form id="authForm">
            ${isRegister ? `
                <div class="form-group">
                    <input type="text" id="fullName" placeholder="Ho ten phu huynh" required>
                </div>
            ` : ''}
            <div class="form-group">
            ...
            <div class="form-group">
                <input type="password" id="password" placeholder="Mat khau" required value="${isRegister ? '' : '123456'}">
            </div>
            ${isRegister ? `
                <div class="form-group">
                    <input type="password" id="confirmPassword" placeholder="Nhap lai mat khau" required>
                </div>
                <div class="form-group">
                    <input type="text" id="phone" placeholder="So dien thoai (khong bat buoc)">
                </div>
            ` : ''}
            <div id="authError" class="hidden auth-error"></div>
            <button type="submit" class="btn btn-primary" id="authBtn">${isRegister ? 'Dang ky' : 'Dang nhap'}</button>
        </form>

        <div class="auth-divider"><span>hoac</span></div>
        <div class="social-login">
            <div id="googleButton" class="${GOOGLE_CLIENT_ID ? '' : 'hidden'}"></div>
            <button type="button" id="facebookLoginBtn" class="btn btn-social ${FACEBOOK_APP_ID ? '' : 'hidden'}">Tiep tuc voi Facebook</button>
            ${(!GOOGLE_CLIENT_ID && !FACEBOOK_APP_ID) ? '<p class="social-note">Social login chua duoc cau hinh.</p>' : ''}
        </div>
    `;
    ...
    document.getElementById('authForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            btn.innerText = isRegister ? 'Dang ky' : 'Dang nhap';
            btn.disabled = false;
        }
    });

    initGoogleButton().catch((error) => showError(`Khong tai duoc Google login: ${error.message}`));
}
